...
class Cards extends Component {
  render() {
    ...
    const { handleUpdate, handleDelete, data, btBooking, handleFasilitas } =
      this.props;
    return (
      <>
        <Card style={{ width: '200px' }} className='mx-auto'>
          <Card.Img
            variant='top'
            src={`https://devruangrapatp2p.kemkes.go.id/backend1/api/${image_ruangan}`}
          ...
          <Card.Body className='text-center'>
            ...
            <Card.Text className={styles.category}>
              {alamat_gedung} Lantai : {ruangan_lantai}
            </Card.Text>
            <Button
